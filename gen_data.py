...
@jit(nopython = True)
def init_sim(N_STATES, N_TREATED, TREATMENT_YEAR, TREATMENT_EFFECT):
    treat_mat = np.empty((N_STATES, 3))
    # treat_mat[:,0] = np.array([np.repeat([np.nan], N_STATES - N_TREATED),
    #                   np.repeat([1.0], N_TREATED)]).flatten()
    treat_mat[:,0] = np.nan
    treat_mat[0:N_TREATED,0] = 1.0
    np.random.shuffle(treat_mat[:,0])
    year_mu, year_sig = TREATMENT_YEAR
    treat_mat[:,1] = np.random.normal(year_mu, year_sig,
                                    N_STATES) * treat_mat[:,0]
    effect_mu, effect_sig = TREATMENT_EFFECT
    treat_mat[:,2] = np.random.normal(effect_mu, effect_sig, N_STATES)
    return(treat_mat)
...
def sim_data(data, TARGET, N_STATES, N_TREATED, TREATMENT_YEAR, TREATMENT_EFFECT):
            #  EXOG_GROWTH_PCT, EXOG_CONTROL_PCT_EXPLAINED,
            #  SPATIAL_AUTOCORRELATION_PCT,
            #  SECTOR_GROWTH_PCT, SECTOR_GROWTH_PCT_PREDICTORS,
            #  TREATMENT_EFFECT, TREATMENT_EFFECT_PREDICTORS):
    ...
    t_mat = init_sim(N_STATES, N_TREATED, TREATMENT_YEAR, TREATMENT_EFFECT)
